...
#install.packages("birtr")
library(birtr)
icc(mydata)

mdmodel <-TAM:: tam.mml(mydata, irtmodel='RSM')

summary(mdmodel)

plot(mdmodel, type="items", ask=FALSE)
# dev.off();
```
...
plotICC(fit1.Subscale1, legpos=FALSE, ask=FALSE)
plotICC(fit1)
#dev.off();
```
...
lrres <- LRtest(fit1.Subscale1, splitcr = "median", se=TRUE)
lrres
...
plot(fa(mydata,  nfactors=4, fm="pa"))
```
...
fa.diagram(fa(mydata, nfactors=3, fm="pa", rotate="varimax"))
...
#Factor analysis table script paste
# load key packages
...
# factor analysis
res<- fa(mydata, nfactors=3, fm="pa", rotate="varimax")
names(efa)
```
...
omega(mydata, nfactors=2, sl=FALSE)
```
...
CFA.mydata <- '
# factor structure
Subscale1 =~ stni1 + stni2 + stni3 + stni4 + stni5 + stni6 + stni7 + stni10a + stni11a'
...
myKeys <- make.keys(nvar=9,list(Subscale1 = c(1:9),
                                 #Subscale2 =c(11:20),
                               # Subscale3 =c(21:30)),
                    item.labels = colnames(mydata[,1:9])))
...
mydata.scores <- scoreItems(myKeys, mydata[,1:9])
...
#dt2 <- mydata[sample(nrow(mydata)),]
library(data.table)

dt2[1:268,]
#fa.diagram(fa(dt2, fm="pa", rotate="oblique"))
#install.packages("EFAtools")
library(EFAtools)
#PARALLEL(dt2, percent = 95)

mycor <- cor(mydata, method="spearman")
install.packages("nFactors")
library(nFactors)
fa.parallel(mycor)
PARALLEL(mycor, N=500, percent= 95)
fa.diagram(fa(mydata,rotate="Promax"))
fa(mydata, rotate="Promax")
fa.diagram(fa(mydata, rotate="none"))
install.packages="car"
library(car)
KMO(mycor)
cortest.bartlett(mycor)
det(mycor)
parallel<- fa.parallel(mycor, n.iter=500, fa='fa',fm='minres', SMC=TRUE, quant=.95)
parallel
#CLUSTER ANALYSIS
...
sdf<-t(sdf)
head(sdf)
res.dist <- get_dist(sdf, method = "pearson")
